...
cpu_stat=`sar 1 3 | grep Average | awk '{for(i=3;i<=NF;i++) printf $i" "}' ` # CPU State
ram_stat=`free -h | grep Mem | awk '{for(i=2;i<=NF;i++) printf $i" "}' ` # RAM State
swap_stat=`free -h | grep Swap | awk '{for(i=2;i<=NF;i++) printf $i" "}' ` # Swap State
